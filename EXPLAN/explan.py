...
from EXPLAN.treeinterpreter.treeinterpreter import treeinterpreter as ti
from imblearn.over_sampling import SMOTE
from EXPLAN.random_sampling import RandomSampling
from EXPLAN.quartile_discretizer import QuartileDiscretization
from EXPLAN.sturges_discretizer import SturgesDiscretization
from EXPLAN.sample_manipulation import SampleManipulation
...
def DataGeneration(instance2explain, blackbox, dataset, N_samples):
    ...
    temp = blackbox(random_samples)
    surrogate_model.fit(random_samples, blackbox(random_samples))
...
def DataBalancing(blacbox, representative_samples, dataset):
    ...
    balanced_samples, _ = oversampler.fit_resample(representative_samples, blacbox(representative_samples))
...
def InterpretabelModel(instance2explain, blackbox, neighborhood_data, dataset):
    ...
    y_x_bb = blackbox(instance2explain.reshape(1,-1))[0]
    ...
    y_X_bb = blackbox(neighborhood_data)
