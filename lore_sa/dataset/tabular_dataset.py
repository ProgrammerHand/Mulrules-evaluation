...
class TabularDataset(Dataset):
    ...
    # @classmethod
    # def from_csv(cls, filename: str, numeric_col_names, categorical_col_names, class_name: str = None, dropna: bool = True):
    #     """
    ...
    #         raise ValueError("numeric_col_names not given")
    #     if categorical_col_names is None:
    #         raise ValueError("categorical_col_names not given")
    #     df[numeric_col_names] = num_imputer.fit_transform(df[numeric_col_names])
    #     df[categorical_col_names] = cat_imputer.fit_transform(df[categorical_col_names])
    #
    #     if dropna:
    ...
    #     return dataset_obj
    @classmethod
    def from_csv(cls, filename: str, numeric_col_names=None, categorical_col_names=None, class_name: str = None,
                 dropna: bool = True):
        """
        Read a comma-separated values (csv) file into Dataset object.
        :param filename: CSV file path
        :param numeric_col_names: list of numeric columns, can be None
        :param categorical_col_names: list of categorical columns, can be None
        :param class_name: target variable
        :param dropna: whether to drop rows with NaNs
        ...
        if categorical_col_names:
            cat_imputer = SimpleImputer(fill_value="?", strategy='constant')
            df[categorical_col_names] = df[categorical_col_names].astype('object')
            df[categorical_col_names] = cat_imputer.fit_transform(df[categorical_col_names])

        if not numeric_col_names and not categorical_col_names:
            raise ValueError("At least one of numeric_col_names or categorical_col_names must be provided.")
