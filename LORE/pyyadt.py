...
#         filename='yadt_dataset', path='./', sep=';', log=False):
#
#     # --- make a safe per-process working dir and unique base name ---
#     if path is None or path in ('./', 'data', 'data/'):
#         base_dir = os.path.join(tempfile.gettempdir(), f"lore_{os.getpid()}")
#     else:
#         base_dir = path
#     os.makedirs(base_dir, exist_ok=True)
#
#     unique = f"_{os.getpid()}_{int(time.time()*1000)}_{uuid.uuid4().hex[:6]}"
#     base = f"{filename}{unique}"
#
#     data_filename  = os.path.join(base_dir, base + '.data')
#     names_filename = os.path.join(base_dir, base + '.names')
#     tree_filename  = os.path.join(base_dir, base + '.dot')
#
#     # ---- write inputs for YADT ----
#     df.to_csv(data_filename, sep=sep, header=False, index=False)
#
#     with open(names_filename, 'w', newline='') as names_file:
#         for col in columns:
#             col_type = features_type[col]
#             disc_cont = 'discrete' if col in discrete else 'continuous'
#             disc_cont = 'class' if col == class_name else disc_cont
#             names_file.write(f"{col}{sep}{col_type}{sep}{disc_cont}\n")
#
#     # ---- call YADT on our unique files ----
#     cmd = ['yadt/dTcmd', '-fd', data_filename, '-fm', names_filename, '-sep', sep, '-d', tree_filename]
...
#     dt_dot = pydotplus.graph_from_dot_data(open(tree_filename, 'r').read())
#
#     # ---- always try to clean up temp files (don’t crash if locked) ----
#     for f in (data_filename, names_filename, tree_filename):
#         try:
#             if os.path.exists(f):
#                 os.remove(f)
#         except OSError:
