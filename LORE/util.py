...
def get_closest_diffoutcome(df, x, discrete, continuous, class_name, blackbox, label_encoder, distance_function,
    ...
    for z, z1 in zip(df.to_dict('records'), Z):
        ...
        distances.append(d)
        if blackbox(z1.reshape(1, -1))[0] == 0:
...
def dataframe2explain(X2E, dataset, instance_from_encoded, # idx_record2explain
                      blackbox):
    ...
    Z = np.vstack([Z, instance_from_encoded.to_numpy()])
    x = instance_from_encoded.to_numpy()
