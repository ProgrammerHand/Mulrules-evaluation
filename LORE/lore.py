...
def explain(instance_from_encoded, # idx_record2explain
            X2E, dataset, blackbox,
    ...
    dfZ, x = dataframe2explain(X2E, dataset, instance_from_encoded , #idx_record2explain
                               blackbox)
    ...
    # safe_path = path
    # if safe_path is None or safe_path in ('./', 'data', 'data/'):
    #     safe_path = os.path.join(tempfile.gettempdir(), f"lore_{os.getpid()}")
    #     os.makedirs(safe_path, exist_ok=True)
    #
    # dt, dt_dot = pyyadt.fit(
    #     dfZ, class_name, columns, features_type, discrete, continuous,
    #     filename=dataset['name'], path=safe_path, sep=sep, log=log
    # )
    ...
    bb_outcome = blackbox(x.reshape(1, -1))[0]
