...
class dataset_object:
            ...
            if dataset_name not in stat_filename:
                ...
                with open(f'{directory}' + stat_filename, 'r') as file:
                    if drop_cols_datasets:
                        self.raw = pd.read_csv(file, skipinitialspace=True, na_values='?', keep_default_na=True).drop(columns=drop_cols_datasets)
                    else:
                        self.raw = pd.read_csv(file, skipinitialspace=True, na_values='?', keep_default_na=True)
    ...
    def impute_missing(self, num_strat='mean', cat_strat='constant', cat_fill = "?"):
        num_imputer = SimpleImputer(strategy=num_strat)
        cat_imputer = SimpleImputer(fill_value=cat_fill, strategy=cat_strat)
        if self.numeric_col_names:
        ...
        return f"Num_strat: {num_strat}, Cat_strat: {cat_strat}, Cat_fill: {cat_fill}"
    ...
    def label_encode_features(self, X, categorical_cols) -> pd.DataFrame:
        X_copy = X.copy()
    ...
    def label_decode_features(self, X, categorical_cols) -> pd.DataFrame:
        X_copy = X.copy()
        for col in categorical_cols:
            if isinstance(X_copy, pd.DataFrame):  # If X is a DataFrame, use column names
