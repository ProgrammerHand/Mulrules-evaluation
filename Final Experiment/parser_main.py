    ...
    for i, instance_name in enumerate(instance_names, start=1):
        ...
        if not non_dominated_rules1.empty:
            parser_util.plot_rules_comparison(all_rules=df_rules[df_rules['Instance_Name'] == instance_name],
                                          ...
                                          instance_name=instance_name, ideal_rule_idx=ideal_rule_idx, ideal_point=ideal_point, save = True, title="Non_dominated_adult_RF_toy")
        ...
        if not non_dominated_rules1_unique.empty:
            ...
            parser_util.plot_rules_comparison(all_rules=df_rules[df_rules['Instance_Name'] == instance_name],
                                          ...
                                          instance_name=instance_name, ideal_rule_idx=ideal_rule_idx, ideal_point=ideal_point, save = True, title="Non_dominated_unique_adult_RF_toy")
        agg_df = parser_util.build_attr_usage_df(non_dominated_rules1_unique)
        agg_all_dom1.append(agg_df)
        parser_util.plot_feature_usage_heatmap(agg_df, feature_col="Feature", explainer_col="Explainer", count_col="Count", all_features=attributes, vmax = max_rules, title=f"instance_{instance_name}_non_dominated_unique.png", save = False)
