...
def plot_feature_usage_heatmap(df: pd.DataFrame,
                               ...
                               all_features: list,
                               vmax: float = None,
                               title = "Feature Usage Across Explainers",
                               save = False,
                               figsize=(10, 8),
                               cmap='YlOrRd'):
    if df.empty:
    ...
    if heatmap_data is not None and theoretical_max > 0:
        ...
        plt.title(f'Scale: 0 to {theoretical_max}, Actual max: {actual_max_count}')
        plt.tight_layout()
        if save:
...
def plot_rules_comparison(all_rules, filtered_rules, explainer_palette, instance_name, ideal_rule_idx, ideal_point, coord_1 = 'Cov', coord_2 = 'Pre', save = False, title = "Rules plot"):
    plt.figure(figsize=(8,6))
...
def combine_winner_matrices(counts_df, ratio_df):
    combined = counts_df.copy()
    for col in counts_df.columns:
        combined[col] = counts_df[col].astype(str) + " (" + (ratio_df[col] * 100).round(1).astype(str) + "%)"
    return combined
